...
### 公開チェックポイントの取得と検証
- 取得元 URL: RoadLib README で案内されている SharePoint フォルダ `https://whueducn-my.sharepoint.com/:f:/g/personal/2015301610143_whu_edu_cn/EjnKWS_nn5dFtfaKRkCwpTUBrgHChZW7jAMK3yzIXQ1H8Q?e=Dy4uyf` から `segformer_whu.pth` をダウンロードする。
- SharePoint 側が認証付きのため、自動ダウンロードは行わず、ブラウザでサインイン → 「Download」から `segformer_whu.pth` を取得する。VPN/プロキシ環境ではダウンロードが失敗する場合があるため、ホスト側で取得し Docker 実行時に `/workspace/weights` にマウントする運用を推奨。
- 検証手順: ダウンロード後に次のコマンドで整合性を確認し、期待値と一致することを確認する。
  - `sha256sum /workspace/weights/segformer_whu.pth`
  - `md5sum /workspace/weights/segformer_whu.pth`
- 期待値（SharePoint 配布ファイルから取得したハッシュ）
  - SHA256: 未取得（SharePoint がプロキシ越しにアクセスできないためこの環境では算出不可）。ホスト側でダウンロード後に上記コマンドで算出し、以降の検証で再利用できるようメモしておく。
  - MD5: 未取得（同上）。
